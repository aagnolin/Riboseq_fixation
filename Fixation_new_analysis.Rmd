...
```{r codon pause scores bis, include=FALSE}
...
# ## This part is to generate a combined file for all ribosome profiles
# 
# #UvA
# setwd("C:/Users/aagnoli/OneDrive - UvA/RNA sequencing data/Global analysis fixation conditions/Normalized by total number of reads")
# 
# # Load input files
# files <- list.files("PauseMeter_fixation/", pattern = "\\pause_codon_output.csv$", full.names = TRUE)
# 
# # read them all into a list
# data_list <- lapply(files, read_csv)
# 
# sample_names <- c("Cm", "Cm", "DSP", "DSP", "DSP+For", "DSP+For", "For", "For", "MeOH", "MeOH", "Untreated", "Untreated", "Tet", "Tet")
# 
# # add the 'Sample' column using your names
# data_list <- Map(function(df, name) {
#   df %>% mutate(Sample = name)
...
# combined <- do.call(rbind, data_list)
# 
# write_csv(combined, "Combined_data_fixation.csv")
# ##

#UvA
setwd("C:/Users/aagnoli/OneDrive - UvA/RNA sequencing data/Global analysis fixation conditions/Normalized by total number of reads")

# Load gene info Bsubtilis
...
Codon_usage_table <- read_excel("C:/Users/aagnoli/OneDrive - UvA/B. subtilis 168 annotation and genome files/B_subtilis_codon_usage_table.xlsx")

#fixation data
super_codonator_output_filtered <- read_csv("Combined_data_fixation.csv")

#Calculate Codon Pause Score
Codon_pause_score_df_A_site <- super_codonator_output_filtered %>% 
group_by(A_site, Sample) %>% 
summarise(Codon_pause_score_A_site = sum(Pause_score)) %>% 
rename(Codon = A_site)
Codon_pause_score_df_P_site <- super_codonator_output_filtered %>% 
group_by(P_site, Sample) %>% 
summarise(Codon_pause_score_P_site = sum(Pause_score)) %>% 
rename(Codon = P_site)
Codon_pause_score_df_E_site <- super_codonator_output_filtered %>% 
group_by(E_site, Sample) %>% 
summarise(Codon_pause_score_E_site = sum(Pause_score)) %>% 
rename(Codon = E_site)

Codon_pause_score_df <- merge(Codon_pause_score_df_A_site, 
                              Codon_pause_score_df_E_site, by = c("Codon", "Sample")) %>% 
merge(Codon_pause_score_df_P_site, by = c("Codon", "Sample"))
super_codonator_output_filtered_ORFs <- super_codonator_output_filtered %>% 
drop_na(A_site)

Total_counts_ORFs <- sum(super_codonator_output_filtered_ORFs$Norm_count)

Codon_pause_score_df <- Codon_pause_score_df %>% 
mutate(Normalised_codon_pause_score_A_site = Codon_pause_score_df$Codon_pause_score_A_site/Total_counts_ORFs, 
...
Codon_pause_score_final <- merge(Codon_pause_score_df, Codon_usage_table, by = "Codon", all = TRUE) %>% 
select(c(-"Codon_pause_score_A_site", -"Codon_pause_score_P_site", -"Codon_pause_score_E_site"))

#adjust df
combined_longer <- Codon_pause_score_final %>%
  pivot_longer(
    cols = starts_with("Normalised_codon_pause_score_"),
    names_to = "Ribosomal_site",
    values_to = "Normalised_codon_pause_score",
    names_prefix = "Normalised_codon_pause_score_"
  )

#plot score of each codon in each ribosomal site

 p <- ggplot(data = filter(combined_longer, Codon != 'TAA', Codon != "TAG", Codon != "TGA"),
       mapping = aes(x = factor(Ribosomal_site, 
                                levels = c('A_site', 'P_site', 'E_site'), 
                                labels = c('A-site', 'P-site', 'E-site')),
                     y = Codon)) +
  geom_tile(mapping = aes(fill = Normalised_codon_pause_score), linewidth = 0.5, color = "black") +
  scale_fill_gradient(low = 'red', high = 'yellow') +
  theme_bw() +
  scale_y_discrete(expand = c(0,0)) +
  scale_x_discrete(expand = c(0,0)) +
  #geom_text(aes(y = Codon, x = -0.1, hjust = -0.3, nudge_y = 0.3, label = `Amino Acid`, color = `Amino Acid`, size = 0.3), #add amino acid labels but it only works for single plots otherwise the spacing is all screwed
   #         inherit.aes = FALSE, show.legend = FALSE, vjust = 1) +  
  #coord_flip() +
  ...
  panel.grid.major = element_line(colour = "gray89", linetype = "blank"), 
  panel.grid.minor = element_line(linetype = "blank"),
...
#check codon frequency in Bli for reference
plot_codon_freq <- ggplot(data = filter(combined_longer, Codon != 'TAA', Codon != "TAG", Codon != "TGA", Ribosomal_site == "A_site"),
       mapping = aes(x = Ribosomal_site,
                     y = Codon)) + 
  geom_tile(mapping = aes(fill = Usage), linewidth = 0.5, color = "black") +
  scale_fill_gradient(low = 'red', high = 'yellow') +
  scale_x_discrete(expand = c(0,0)) +
  theme_bw() +
  theme(legend.position = "top", 
  ...
  axis.text.x = element_text(size = 12),
  axis.text.y = element_text(size = 12),) +
  labs(x = "Codon Frequency Bli",
  y = "", 
  fill = "Frequency",
  title = "Codon Frequency in Bli genome")


patchwork_pausing <- p | plot_codon_freq +
...
```{r new metagene method - END OF GENES}
library(tidyverse)
